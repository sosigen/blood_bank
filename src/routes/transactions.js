...
router.post('/api/transactions', (req, res) => {
	//`transaction_id`, `transaction_id`, `employee_id`, `transaction_date`, `quantity`, `recipient_id`
...
router.get('/api/transactions/all', (req, res) => {
	const gettransactionsQuery = `SELECT transaction_id, donation_id, DATE_FORMAT(transaction_date,'%d/%m/%Y'), quantity, recipient_id FROM transactions`;
